          ...
          <nav aria-label="breadcrumb" class="breadcrumb-header float-start float-lg-end">
            <ol class="breadcrumb">
              <li class="breadcrumb-item">
                <a href="{{ route('officers.index') }}">Data Petugas</a>
              </li>
    ...
    <section class="section">
      <div class="card">
        <div class="card-header">
            <button class="btn btn-primary btn-sm" onclick="window.location='/officers/create'"><i class="fas fa-plus"></i> Tambah Data</button>
        </div>
        <div class="card-body">
          <table class="table categories-table" id="table1">
            ...
            @foreach ($officers as $row)
              <tr>
                ...
                <td>{{ $row->telephone }}</td>
                <td>{{ $row->hasRole('admin') ? 'Admin' : 'Kasir' }}</td>
                <td>
                  <button class="btn btn-warning btn-sm mb-2" onclick="window.location='/officers/{{ $row->id }}/edit'"><i class="fas fa-edit"></i> Edit</button>
