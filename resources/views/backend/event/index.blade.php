    ...
    <section class="section">
      <div class="card">
        <div class="card-header">
            <button class="btn btn-primary btn-sm" onclick="window.location='/event/create'"><i class="fas fa-plus"></i> Tambah Data</button>
        </div>
        <div class="card-body">
                ...
                <td>
                  @if ($row->images->count() > 0)
                      @foreach ($row->images as $image)
                          <img src="{{ asset('storage/event/'.$image->path) }}" class="mb-2" width="100%" alt="image">
                      @endforeach
                  @endif
                </td>
                <td>{{ $row->title }}</td>
