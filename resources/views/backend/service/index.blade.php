    ...
    <section class="section">
      <div class="card">
        <div class="card-header">
            <button class="btn btn-primary btn-sm" onclick="window.location='/services/create'"><i class="fas fa-plus"></i> Tambah Data</button>
        </div>
        <div class="card-body">
          <table class="table categories-table" id="table1">
            <thead>
              <tr>
                <th width="5%">No</th>
                <th>Nama</th>
                <th>Harga Harian (/jam)</th>
                <th>Harga Member (/bulan)</th>
                <th width="20%">Aksi</th>
              </tr>
            </thead>
            <tbody>
            @foreach ($services as $row)
              <tr>
                ...
                <td>{{ $row->name }}</td>
                <td>{{ number_format($row->price_daily, 0, ',', '.') }}</td>
                <td>{{ number_format($row->price_member, 0, ',', '.') }}</td>
                <td>
                  <button class="btn btn-warning btn-sm mb-2" onclick="window.location='/services/{{ $row->id }}/edit'"><i class="fas fa-edit"></i> Edit</button>
